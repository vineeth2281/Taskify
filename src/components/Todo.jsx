...
import WeekCalendarView from "../components/Calenderview";
import {
  PlusCircle,
  CheckCircle,
  Circle,
...
const Todo = ({ user }) => {
  ...
  const [filter, setFilter] = useState("all"); // alfcall, priority, today, upcoming
  ...
  const [selectedTodoForDependencies, setSelectedTodoForDependencies] =
    useState(null);
  ...
  // Function to get AI tag suggestion
  const getAITagSuggestion = async (text) => {
    try {
      const response = await fetch(
        "https://api-inference.huggingface.co/models/facebook/bart-large-mnli",
        {
          ...
          body: JSON.stringify({
            inputs: text,
            parameters: {
              candidate_labels: tags, // Use existing tags array
            },
          }),
        }
      );
      ...
      const suggestedTag = result.labels[result.scores.indexOf(highestScore)];
  ...
  // Modified add function with AI tagging
  const add = async () => {
    ...
    try {
      // Get AI tag suggestion
      const suggestedTag = await getAITagSuggestion(value);
      ...
      const newTodo = {
        ...
        isPriority: false,
        dueDate: "",
        tag: suggestedTag, // Use AI suggested tag
        timeSpent: 0,
        isTracking: false,
        startTime: null,
        subtasks: [],
      ...
      prevTodos.map((todo) => {
        if (todo.id === id) {
          if (!todo.isTracking) {
            ...
            const timeSpent =
              todo.timeSpent + (new Date() - new Date(todo.startTime));
  ...
  const canCompleteTask = (todoId) => {
    ...
    return todo.dependencies.every(
      (depId) => todoList.find((t) => t.id === depId)?.completed
    );
  };
  ...
  const getDependencyDetails = (todoId) => {
    const pendingDeps = todoList
      .filter(
        (todo) => todoId.dependencies?.includes(todo.id) && !todo.completed
      )
      .map((todo) => todo.text);
    return pendingDeps;
  };
    ...
    return todoList.filter((todo) => {
      ...
      if (searchQuery) {
        ...
        const matchesSearch =
          ...
          todo.subtasks.some((subtask) =>
            subtask.text.toLowerCase().includes(searchLower)
          );
      ...
      if (showAdvancedFilters) {
        ...
        if (
          ...
          !advancedFilters.selectedTags.includes(todo.tag)
        ) {
          return false;
        }
        ...
        if (advancedFilters.completionStatus !== "all") {
          if (
            ...
            return false;
          if (
            advancedFilters.completionStatus === "incomplete" &&
            todo.completed
          )
            return false;
        }
  ...
  return (
    <div
      className={`h-[800px] p-4 flex justify-center items-start transition-colors duration-200 ${
        isDarkMode
    ...
    >
      <div
        className={`w-full h-full max-w-7xl flex flex-col p-6 rounded-xl shadow-lg border border-blue-400  transition-colors duration-200 overflow-hidden $ ${
      ...
      >
            ...
            >
              <i
                className={`fas fa-calendar ${
                  showWeekView ? "text-white" : "text-gray-600"
                }`}
              ></i>
        ...
        <div
          className={`  min-h-screen p-8 flex justify-center items-start transition-colors duration-200 ${
            isDarkMode
              ? "bg-gradient-to-br from-gray-900 to-gray-800"
              : "bg-gradient-to-br from-blue-50 to-purple-50"
          }`}
        >
          <div
            className={` border-blue-400 w-11/12 max-w-4xl flex flex-col p-6 rounded-xl shadow-lg transition-colors duration-200  min-h-[500px] ${
              isDarkMode ? "bg-gray-800" : "bg-white"
            }`}
          >
            ...
            <div className=" border-blue-400 mt-6">
              <div className="relative">
                ...
                >
                  {isTaggingLoading ? (
                    <div className="animate-spin rounded-full h-6 w-6 border-2 border-b-transparent border-blue-500" />
                  ) : (
            ...
            {/* Todo List */}
            <div className=" border-blue-400 mt-6 space-y-2 max-h-[300px] overflow-y-auto">
              {filteredTodos.map((item) => (
                ...
                >
                  ...
                  <div className="flex-auto">
                    <div className="flex items-center gap-2 py-2 min-h-2000">
                    ...
                    <div className=" border-blue-400 ml-6 mt-2 space-y-0 min-h-600">
                    ...
                    {/* Dependencies Modal */}
                    {showDependencyModal &&
                      selectedTodoForDependencies === item.id && (
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      className={`p-1 hover:bg-gray-100 rounded ${
                        item.isTracking ? "text-green-500" : "text-gray-400"
                      }`}
                    >
                    ...
                    >
                      ...
                      <option value="FREQ=WEEKLY;BYDAY=MO,TH">
                        Every Monday and Thursday
                      </option>
                      <option value="FREQ=WEEKLY;BYDAY=MO">Every Monday</option>
                      <option value="FREQ=DAILY">Daily</option>
                      <option value="FREQ=MONTHLY;BYDAY=1MO">
              ...
              <div className="mt-6">
                <WeekCalendarView
                  ...
                  onEventAdd={(eventDetails) => {
                    // Add new todo with scheduling info
                    const newTodo = {
                      ...eventDetails,
                      userEmail: user.email,
                      completed: false,
                  ...
                  onEventUpdate={(eventId, updates) => {
                    ...
                    updateDoc(doc(db, "todos", eventId), updates);
                  }}
                />
              </div>
            )}
